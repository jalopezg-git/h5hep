...
///   })
/// ```
template <ColumnModel M> struct SchemaBuilder {
    static constexpr ColumnModel GetColumnModel() { return M; }

    template <typename T>
    static std::shared_ptr<PrimitiveNode<M, T>> MakePrimitiveNode(std::string_view name = "(anonymous)",
                                                                  size_t offset = 0) {
    ...
    template <typename T>
    static std::shared_ptr<StructNode<M, T>> MakeStructNode(std::string_view name,
                                                            typename StructNode<M, T>::MemberList_t &&v) {
        return std::make_shared<StructNode<M, T>>(name, 0UL, v);
    }

    template <typename T>
    static std::shared_ptr<CollectionNode<M, typename T::Value_t>>
    MakeCollectionNode(std::string_view name, size_t offset, std::shared_ptr<T> inner) {
