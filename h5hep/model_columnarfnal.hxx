...
namespace schema {
template <> class Node<ColumnModel::COLUMNAR_FNAL> : public NodeBase {
  protected:
...
template <typename T> class PrimitiveNode<ColumnModel::COLUMNAR_FNAL, T> : public Node<ColumnModel::COLUMNAR_FNAL> {
  public:
    using Value_t = T;
    ...
    void Realize(ReaderWriter &rw, void *privateData) override {
        ...
        if (info.dcpl != H5I_INVALID_HID) {
            ...
            datasetId = H5Dcreate(info.parentGroup, name.c_str(), typeId, spaceId, H5P_DEFAULT, info.dcpl, H5P_DEFAULT);
        } else {
...
template <typename T> class StructNode<ColumnModel::COLUMNAR_FNAL, T> : public Node<ColumnModel::COLUMNAR_FNAL> {
  public:
    using Value_t = T;
    ...
    size_t ReadExtents(const std::vector<Span> &extents, void *buf) override {
        size_t nElements =
            std::accumulate(extents.begin(), extents.end(), 0, [](size_t a, const Span &b) { return a + b.len; });
        ...
        for (const auto &I : children) {
            ...
            internal::CopyElements(reinterpret_cast<unsigned char *>(buf) + I->offset, columnBuffer.get(), c, I->size,
                                   size, I->size);
    ...
    size_t WriteExtents(const std::vector<Span> &extents, const void *buf) override {
        ...
        for (const auto &I : children) {
            ...
            internal::CopyElements(columnBuffer.get(), reinterpret_cast<const unsigned char *>(buf) + I->offset,
...
template <typename T> class CollectionNode<ColumnModel::COLUMNAR_FNAL, T> : public Node<ColumnModel::COLUMNAR_FNAL> {
    template <typename U> struct SequentialColumnReader {
        ...
        void ReadNextChunk() { nEltsInChunk = column.ReadExtents(std::vector<Span>{{offset, chunkSize}}, chunk.get()); }
  ...
  public:
    ...
    CollectionNode(std::string_view name, size_t offset, NodePtr_t inner)
        ...
        indexColumnReader =
            std::make_unique<SequentialColumnReader<IndexColumnValue_t>>(*indexColumn.get(), defaultChunkSize);
    }
    ...
    size_t WriteExtents(const std::vector<Span> &extents, const void *buf) override {
        ...
        size_t nElements =
            std::accumulate(extents.begin(), extents.end(), 0, [](size_t a, const Span &b) { return a + b.len; });
        size_t szLargestCollection =
            std::accumulate(hvls, hvls + nElements, 0, [](size_t a, const hvl_t &b) { return std::max(a, b.len); });
...
template <> class ReaderWriterSpec<ColumnModel::COLUMNAR_FNAL> : public ReaderWriter {
    ...
    struct Metadata_t {
        ...
        Metadata_t(hid_t location, const WriteProperties &props) : attrs{props.GetChunkSize(), 0U} {
