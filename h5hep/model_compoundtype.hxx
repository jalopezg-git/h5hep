...
namespace schema {
template <> class Node<ColumnModel::COMPOUND_TYPE> : public NodeBase {
  public:
...
template <typename T> class PrimitiveNode<ColumnModel::COMPOUND_TYPE, T> : public Node<ColumnModel::COMPOUND_TYPE> {
  public:
    using Value_t = T;
...
template <typename T> class StructNode<ColumnModel::COMPOUND_TYPE, T> : public Node<ColumnModel::COMPOUND_TYPE> {
  public:
    using Value_t = T;
...
template <typename T> class CollectionNode<ColumnModel::COMPOUND_TYPE, T> : public Node<ColumnModel::COMPOUND_TYPE> {
  public:
    using Value_t = T;
    ...
    void InitializeValue(void *p) override {
        reinterpret_cast<CollectionBase *>(p)->assign(
            ...
            [](void *p, void *data) {
                ...
                H5Dvlen_reclaim(that->children.back()->typeId, that->readerWriter->GetSpaceId(), H5P_DEFAULT, p);
            },
            this);
    }
...
template <> class ReaderWriterSpec<ColumnModel::COMPOUND_TYPE> : public ReaderWriter {
  public:
    ...
    ReaderWriterSpec(std::shared_ptr<H5Location> location, std::shared_ptr<schema::NodeBase> schemaRoot,
        ...
        datasetId =
            H5Dcreate(location->GetHandle(), root->name.c_str(), root->typeId, spaceId, H5P_DEFAULT, dcpl, H5P_DEFAULT);
    }
    ...
    size_t GetNEntries() const override { return internal::GetDataspaceSize(spaceId); }
